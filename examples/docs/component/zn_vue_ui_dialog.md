...
## 安装 ##
```
npm i @mobile-component/zn_vue_ui_dialog@latest --save
```
## 导入工程 ##
```
import ZnDialog from '@mobile-component/zn_vue_ui_dialog'
Vue.use(ZnDialog)
```
## 用法说明 ##
:::demo
...
|--------|--------|---------|----------|--------------|-------|
|show|弹窗展示隐藏|Boolean|true|false|是|
|title|弹窗标题|String|-|提示|否|
|width|对话框宽度，对话框的宽度是响应式的，当屏幕尺寸小于 768px 时，宽度会变为自动auto。当其值不大于 100 时以百分比显示，大于 100 时为像素|Number|-|520|否|
|footBtnHide|隐藏底部按钮|Boolean|true|false|否|
|closeIconShow|隐藏右上角关闭按钮|Boolean|-|提示|否|
|confirmText|确认按钮 文案|String|-|确认|否|
|confirmBtnShow|确认按钮是否显示|Boolean|false|true|否|
|cancelText|取消按钮 文案|String|-|取消|否|
|cancelBtnShow|取消按钮是否显示|Boolean|false|true|否|
|modalClassName|设置对话框容器.ivu-modal-wrap的类名，可辅助实现垂直居中等自定义效果|String|-|-|否|
...
|-------------|---------|----------|
|cancel|监听点击取消按钮事件|-|
|iconClose|监听点击右上角关闭按钮事件|-|
|confirm|监听点击确定按钮事件|-|

### slot ###
|名称       |说明     |
|-------------|---------|
|zn-footer-btn|自定义页脚内容|

...
// }
/**
 * @param {Array} list 通过接口数据得到路由列表里对应的菜单列表
 * @returns {Array}
 */
export const getMenuByRouter = (list, access) => {
  ...
  // 获取菜单
  forEach(list, item => {
    let meta = { title: item.title, icon: `${item.icon}` }
    let obj = {
      icon: item.icon && `${item.icon}`, // (item.meta && item.meta.icon) || '',
      name: item.code, // 取路由code ace中路径编码作为索引的唯一值必须配置
      meta: meta
    }
    if (hasChild(item)) {
      obj.children = getMenuByRouter(item.subs)
    }
    res.push(obj)
  })
